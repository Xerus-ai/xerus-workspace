#!/usr/bin/env python3
"""
Xerus IPC MCP Server - Inter-Process Communication between Claude Code instances
Uses TCP sockets for agent-to-agent communication within Daytona sandbox
"""
...
# Configuration
IPC_HOST = "127.0.0.1"  # Localhost for intra-sandbox communication
IPC_PORT = 9876  # Default port for IPC broker
...
class MessageBroker:
    ...
    def _hash_token(self, token: str) -> str:
        ...
        # Load salt from environment, fallback only in development
        salt = os.environ.get("IPC_TOKEN_SALT")
        if not salt:
            if os.environ.get("NODE_ENV") == "production" or os.environ.get("XERUS_ENV") == "production":
                raise RuntimeError("IPC_TOKEN_SALT is required in production")
    ...
    def _process_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if action == "register":
                ...
                # Mark messages as read in database (batch UPDATE)
                if self.db_path and messages:
                    try:
                        conn = self._get_connection()
                        cursor = conn.cursor()

                        # Batch update: collect all timestamps and execute single UPDATE
                        ...
                        if timestamps:
                            ...
                            cursor.execute(f'''
                                UPDATE messages
                                SET read_flag = 1
                                WHERE to_id = ? AND timestamp IN ({placeholders}) AND read_flag = 0
                            ''', [resolved_id] + timestamps)
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "instance_id": {
                        "type": "string",
                        "description": "Unique identifier for this instance (e.g., 'agent-1', 'manager')"
                    }
                },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    if name == "register":
        ...
        for msg in messages:
            ...
            # For now, just acknowledge receipt for known senders
            if sender != "system":
        ...
        import time
        config_dir = os.path.expanduser("~/.xerus/ipc")
        os.makedirs(config_dir, exist_ok=True)
        config_file = os.path.join(config_dir, "auto_check_config.json")
...
async def run_server():
    ...
    logger.info("Starting Xerus IPC MCP Server")
