...
HITL_RULES_FILE="$WORKSPACE_ROOT/agents/$AGENT_SLUG/.hitl_required"
if [ -f "$HITL_RULES_FILE" ]; then
  if grep -qxF "$TOOL_NAME" "$HITL_RULES_FILE" 2>/dev/null; then
    TOOL_USE_ID="${CLAUDE_TOOL_USE_ID:-}"
    APPROVED_FILE="$WORKSPACE_ROOT/agents/$AGENT_SLUG/.hitl_approved/$TOOL_USE_ID"
    if [ -z "$TOOL_USE_ID" ] || [ ! -f "$APPROVED_FILE" ]; then
      ...
      mkdir -p "$WORKSPACE_ROOT/agents/$AGENT_SLUG/.hitl_pending"
      TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)
      echo "{\"tool\":\"$TOOL_NAME\",\"agent\":\"$AGENT_SLUG\",\"tool_use_id\":\"$TOOL_USE_ID\",\"timestamp\":\"$TIMESTAMP\"}" \
        > "$WORKSPACE_ROOT/agents/$AGENT_SLUG/.hitl_pending/$TOOL_USE_ID.json"
      exit 1
    fi
  fi
fi
...
# Log tool usage
TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)
echo "{\"event\":\"tool_use\",\"tool\":\"$TOOL_NAME\",\"agent\":\"$AGENT_SLUG\",\"timestamp\":\"$TIMESTAMP\"}" >> "$WORKSPACE_ROOT/shared/activity.jsonl"
