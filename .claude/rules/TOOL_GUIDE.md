...
| Check integrations (Notion, Gmail, …) | `mcp__platform__search_tools` — results include `is_connected` |
| Use a connected app | Its MCP tools: `mcp__{app_slug}__*` (e.g. `mcp__notion__*`) |
...
- NEVER query `workspace.db` for integrations/tool connections — connection state lives in the platform backend; use `mcp__platform__search_tools`
